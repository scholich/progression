...
def _wrapper_func(func, args, shared_mem_run, shared_mem_pause, interval, log_queue, sigint, sigterm, name,
...
class Loop(object):
    ...
    _wrapper_func = staticmethod(_wrapper_func)
...
class Progress(Loop):
    ...
    _calc = staticmethod(_calc)
    show_stat_wrapper_multi = staticmethod(show_stat_wrapper_multi)
    show_stat = show_stat
    show_stat_wrapper = staticmethod(show_stat_wrapper)
        ...
        try:
            for c in count:
                if not isinstance(c, Synchronized):
                    raise ValueError(
                        "Each element of 'count' must be if the type multiprocessing.sharedctypes.Synchronized")
            self.is_multi = True
        except TypeError:
...
_progress_bar_show_stat = ProgressBar.show_stat
...
_progress_bar_counter_show_stat = ProgressBarCounter.show_stat
...
def check_process_termination(proc, prefix, timeout, auto_kill_on_last_resort=False):
    ...
    while True:
        ...
        try:
            answer = input(
                "press 'enter' to send SIGTERM, enter 'k' to send SIGKILL or enter 'ignore' to not bother about the process anymore")
        except Exception as e:
...
def print_static_method_extraction():
    """Convenience function to print static methods.

     Can be copy-pasted at the end of the code to extract them.
     Unfortunately, that did not appear to solve the problem.
    ...
    print("\n".join([key for key in ProgressBarFancy.__class__.__dict__.keys()]))
    clsmembers = inspect.getmembers(sys.modules[__name__], predicate=inspect.isclass)
    # static_members = []
    print(clsmembers)
    for cls in clsmembers:
        for m in inspect.getmembers(cls[1], predicate=inspect.isfunction):
            try:
                # print((cls[0], m[0], type(ProgressBarFancy.__dict__[m[0]])))
                print("_" + cls[0].lower() + "_" + m[0] + " = " + cls[0] + "." + m[0])
                # static_members.append(m[1])
            except:
                pass


if __name__ == '__main__':
    # print_static_method_extraction()
    pass
